...
/// Begins new work from the latest shared base.
pub fn run(repo: &crate::git::Repo, name: &str) -> Result<()> {
    if status::is_dirty(repo)? {
        return Err(UngitError::Precondition(
            "there are unsaved changes; save them before beginning new work".to_string(),
        ));
    }
        ...
        match (relation.ahead, relation.behind) {
            ...
            _ => {
                return Err(UngitError::Refused(
                    "the base branch contains local work; ungit will not begin new work from a divergent base".to_string(),
                ));
            }
        }
    }

    output::step(format!("Beginning '{name}' from {remote_base}..."));
    branch::create_and_switch(repo, name, &remote_base)?;
    output::success(format!("Began '{name}'."));
    Ok(())
}
